...
export function mergeChatCountArrays(arr1, arr2) {
  ...
  function mergeTheArray(arr) {
    ...
    for (const element of arr) {
      ...
      let value = element.chat_count || element.chatCount;
  ...
  context?.citations?.forEach((citation, index) => {
    ...
    if (replacedContent.includes(docKey)) {
      try {
        ...
        let linkLabel = String(links.length + 1).split('').map((digit) => superscriptMap[digit]).join('');
