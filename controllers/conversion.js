...
router.post('/csv_to_json', multer({ limits: { fileSize: FILE_SIZE_LIMIT } }).array('file'), (req, res) => {
  const file = base64ToText(req.body.file);
  const result = Papa.parse(file, { skipEmptyLines: true });
  res.json(result.data);
});

router.post('/yaml_to_json', multer({ limits: { fileSize: FILE_SIZE_LIMIT } }).array('file'), (req, res) => {
  const file = base64ToText(req.body.file);
...
router.post('/string-to-array', [body('data').isString().withMessage('data must be a string')], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).json({ errors: errors.array() });
  }
...
router.post(
  '/array-to-xlsx',
  [body('data').isArray().withMessage('data must be an array of string arrays')],
  async (req, res) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Sheet1');
